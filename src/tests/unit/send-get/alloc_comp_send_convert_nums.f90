...
program alloc_comp_send_convert_nums
  ...
  associate(me => this_image(), np => num_images())
    ...
    if (me == 1) then
      ...
      if (obj%int_scal_k4 /= int_scal_k4) error stop 'send scalar int kind=4 to kind=4 self failed.'

      obj[1]%int_scal_k4 = int_scal_k1
      print *, obj%int_scal_k4
      if (obj%int_scal_k4 /= int_scal_k4) error stop 'send scalar int kind=1 to kind=4 self failed.'

      obj[1]%int_scal_k1 = int_scal_k4
      print *, obj%int_scal_k1
      if (obj%int_scal_k1 /= int_scal_k1) error stop 'send scalar int kind=4 to kind=1 self failed.'

      obj[1]%int_k1(:) = int_k1
      print *, obj%int_k1
      if (any(obj%int_k1 /= int_k1)) error stop 'send int kind=1 to kind=1 self failed.'

      obj[1]%int_k4(:) = int_k4
      print *, obj%int_k4
      if (any(obj%int_k4 /= int_k4)) error stop 'send int kind=4 to kind=4 self failed.'

      obj[1]%int_k4(:) = int_k1
      print *, obj%int_k4
      if (any(obj%int_k4 /= int_k4)) error stop 'send int kind=1 to kind=4 self failed.'

      obj[1]%int_k1(:) = int_k4
      print *, obj%int_k1
      if (any(obj%int_k1 /= int_k1)) error stop 'send int kind=4 to kind=1 self failed.'
    else if (me == 2) then ! Do the real copy to self checks on image 2
      obj[2]%real_scal_k4 = real_scal_k4
      print *, obj%real_scal_k4
      if (abs(obj%real_scal_k4 - real_scal_k4) > tolerance4) error stop 'send scalar real kind=4 to kind=4 self failed.'

      obj[2]%real_scal_k8 = real_scal_k8
      print *, obj%real_scal_k8
      if (abs(obj%real_scal_k8 - real_scal_k8) > tolerance8) error stop 'send scalar real kind=8 to kind=8 self failed.'

      obj[2]%real_scal_k8 = real_scal_k4
      print *, obj%real_scal_k8
      if (abs(obj%real_scal_k8 - real_scal_k8) > tolerance4to8) error stop 'send scalar real kind=4 to kind=8 self failed.'

      obj[2]%real_scal_k4 = real_scal_k8
      print *, obj%real_scal_k4
      if (abs(obj%real_scal_k4 - real_scal_k4) > tolerance4) error stop 'send scalar real kind=8 to kind=4 self failed.'

      obj[2]%real_k4(:) = real_k4
      print *, obj%real_k4
      if (any(abs(obj%real_k4 - real_k4) > tolerance4)) error stop 'send real kind=4 to kind=4 self failed.'

      obj[2]%real_k8(:) = real_k8
      print *, obj%real_k8
      if (any(abs(obj%real_k8 - real_k8) > tolerance8)) error stop 'send real kind=8 to kind=8 self failed.'

      obj[2]%real_k8(:) = real_k4
      print *, obj%real_k8
      if (any(abs(obj%real_k8 - real_k8) > tolerance4to8)) error stop 'send real kind=4 to kind=8 self failed.'

      obj[2]%real_k4(:) = real_k8
      print *, obj%real_k4
      if (any(abs(obj%real_k4 - real_k4) > tolerance4)) error stop 'send real kind=8 to kind=4 self failed.'
    end if
    ...
    if (me == 2) then
      print *, obj%int_scal_k1
      if (obj%int_scal_k1 /= int_scal_k1) error stop 'send scalar int kind=1 to kind=1 to image 2 failed.'

      print *, obj%int_scal_k4
      if (obj%int_scal_k4 /= int_scal_k4) error stop 'send scalar int kind=4 to kind=4 to image 2 failed.'

      print *, obj%int_k1
      if (any(obj%int_k1 /= int_k1)) error stop 'send int kind=1 to kind=1 to image 2 failed.'

      print *, obj%int_k4
      if (any(obj%int_k4 /= int_k4)) error stop 'send int kind=4 to kind=4 to image 2 failed.'

      print *, obj%real_scal_k4
      if (abs(obj%real_scal_k4 - real_scal_k4) > tolerance4) error stop 'send scalar real kind=4 to kind=4 to image 2 failed.'

      print *, obj%real_scal_k8
      if (abs(obj%real_scal_k8 - real_scal_k8) > tolerance8) error stop 'send scalar real kind=8 to kind=8 to image 2 failed.'

      print *, obj%real_k4
      if (any(abs(obj%real_k4 - real_k4) > tolerance4)) error stop 'send real kind=4 to kind=4 to image 2 failed.'

      print *, obj%real_k8
      if (any(abs(obj%real_k8 - real_k8) > tolerance8)) error stop 'send real kind=8 to kind=8 to image 2 failed.'
    end if
    ...
    if (me == 2) then
      print *, obj%int_scal_k4
      if (obj%int_scal_k4 /= int_scal_k4) error stop 'send scalar int kind=1 to kind=4 to image 2 failed.'

      print *, obj%int_scal_k1
      if (obj%int_scal_k1 /= int_scal_k1) error stop 'send scalar int kind=4 to kind=1 to image 2 failed.'

      print *, obj%int_k4
      if (any(obj%int_k4 /= int_k4)) error stop 'send int kind=1 to kind=4 to image 2 failed.'

      print *, obj%int_k1
      if (any(obj%int_k1 /= int_k1)) error stop 'send int kind=4 to kind=1 to image 2 failed.'

      print *, obj%real_scal_k8
      if (abs(obj%real_scal_k8 - real_scal_k8) > tolerance4to8) error stop 'send scalar real kind=4 to kind=8 to image 2 failed.'

      print *, obj%real_scal_k4
      if (abs(obj%real_scal_k4 - real_scal_k4) > tolerance4) error stop 'send scalar real kind=8 to kind=4 to image 2 failed.'

      print *, obj%real_k8
      if (any(abs(obj%real_k8 - real_k8) > tolerance4to8)) error stop 'send real kind=4 to kind=8 to image 2 failed.'

      print *, obj%real_k4
      if (any(abs(obj%real_k4 - real_k4) > tolerance4)) error stop 'send real kind=8 to kind=4 to image 2 failed.'
    end if
    ...
    if (me == 2) then
      print *, obj%int_k4
      if (any(obj%int_k4 /= int_scal_k4)) error stop 'send int scal kind=4 to array kind=4 to image 2 failed.'

      print *, obj%int_k1
      if (any(obj%int_k1 /= int_scal_k1)) error stop 'send int scal kind=1 to array kind=1 to image 2 failed.'

      print *, obj%real_k8
      if (any(abs(obj%real_k8 - real_scal_k8) > tolerance8)) error stop 'send real kind=8 to array kind=8 to image 2 failed.'

      print *, obj%real_k4
      if (any(abs(obj%real_k4 - real_scal_k4) > tolerance4)) error stop 'send real kind=4 to array kind=4 to image 2 failed.'
    end if
    ...
    if (me == 2) then
      print *, obj%int_k4
      if (any(obj%int_k4 /= int_scal_k4)) error stop 'send int scal kind=1 to array kind=4 to image 2 failed.'

      print *, obj%int_k1
      if (any(obj%int_k1 /= int_scal_k1)) error stop 'send int scal kind=4 to array kind=1 to image 2 failed.'

      print *, obj%real_k8
      if (any(abs(obj%real_k8 - real_scal_k8) > tolerance8)) error stop 'send real kind=4 to array kind=8 to image 2 failed.'

      print *, obj%real_k4
      if (any(abs(obj%real_k4 - real_scal_k4) > tolerance4)) error stop 'send real kind=8 to array kind=4 to image 2 failed.'
    end if
    ...
    if (me == 2) then
      print *, obj%int_k4
      if (any(obj%int_k4 /= INT(real_scal_k4, 4))) error stop 'send real scal kind=4 to int array kind=4 to image 2 failed.'

      print *, obj%int_k1
      if (any(obj%int_k1 /= INT(real_scal_k4, 1))) error stop 'send real scal kind=1 to int array kind=1 to image 2 failed.'

      print *, obj%real_k8
      if (any(abs(obj%real_k8 - int_scal_k4) > tolerance4to8)) error stop 'send int kind=4 to real array kind=8 to image 2 failed.'

      print *, obj%real_k4
      if (any(abs(obj%real_k4 - int_scal_k4) > tolerance4)) error stop 'send int kind=4 to real array kind=4 to image 2 failed.'
    end if
    ...
    if (me == 1) then
      ...
      print *, obj%int_k4
      if (any(obj%int_k4 /= [int_k4(1), -1, int_k4(2), -1, int_k4(3)])) error stop 'send strided int kind=4 to kind=4 self failed.'

      obj%int_k4 = -2
      obj[1]%int_k4(2::2) = int_k1(4:5)
      print *, obj%int_k4
      if (any(obj%int_k4 /= [-2, int_k4(4), -2, int_k4(5), -2])) error stop 'send strided int kind=1 to kind=4 self failed.'
...
!        & error stop 'send strided with temp real kind=8 to kind=8 self failed.'
    ...
    if (me == 2) then
      print *, obj%int_k4
      if (any(obj%int_k4 /= [15, -1, 13, -1, 11])) error stop 'strided send int kind=1 to kind=4 to image 2 failed.'
    ...
    if (me == 2) then
      print *, obj%int_k4
      if (any(obj%int_k4 /= [15, -1, 13, -1, 11])) error stop 'strided send real kind=4 to int kind=4 to image 2 failed.'
