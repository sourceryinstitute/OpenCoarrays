...
int main(int argc, char* argv[])
{
   ...
   // initalize with values expected from command line: mpirun -np 2 ./co_sum
   int *expected{ new int[LENGTH]{50, 52, 54, 56, 58} };   
   ...
   // fill out the source array with values
   for (int i = 0; i < LENGTH; ++i) {
      arr[i] = (img + 1) * 10 + i;
   }
   
   // necessary for caf_co_sum() call
   int stat;
   size_t errmsg_len = 20;
   char *errmsg = new char[errmsg_len];
   int result_image = 1;          // image where result should be stored

   caf_co_sum(&arr_desc, result_image, &stat, errmsg, errmsg_len);
