...
module run_size
    implicit none
        ...
        integer(8), codimension[*] ::  nx, ny, nz, nsteps, output_step
        integer(8), codimension[*] :: my, mx, first_y, last_y, first_x, last_x
        real(8), codimension[*] :: cpu_time, tran_time, sync_time, total_time
        real(8), codimension[*] :: max_cpu_time, max_tran_time, max_sync_time, max_total_time
        real(8), codimension[*] :: min_cpu_time, min_tran_time, min_sync_time, min_total_time
...
!-------------   out-of-place transpose data_s --> data_r  ----------------------------
 ...
 subroutine transpose_X_Y
    ...
    call copy3 (    u(1,1,first_x,1+(my_node-1)*my) &                   !-- intra-node transpose
                ...
                ,   nz*3, 1, 1        &                                 !-- note: only 3 of 4 words needed
                ,   mx, nz*4, nz*4*my &
                ,   my, nz*4*mx, nz*4 )
...
subroutine transpose_Y_X
    ...
    call copy3 (   ur(1,1,first_y,1+(my_node-1)*mx) &                   !-- intra-node transpose
                ...
                ,   nz*4, 1, 1        &                                 !-- note: all 4 words needed
                ,   my, nz*4, nz*4*mx &
                ,   mx, nz*4*my, nz*4 )
...
!(*------------------------- un( K ) = conjg( un( -K ) ) ---------------------*)
 ...
 subroutine define_field
   ...
   phi = RAND(12345+53*my_node)     !(* seed random numbers for each image *)
   time = 0
...
!(*---------------------------   LEAVING FOURIER WAVE SPACE  --------------------------*)
        ...
        do i = 1, 3
            call cfft ( ny, nz, u(1,i,x,1), nz*4*mx, 1, trigy, 1 );    end do
	end do
 ...
 subroutine  phase2
  ...
  do  y = first_y, last_y
      ...
      do  i = 1, 3
           call cfft ( nz, nx/2, ur(1,i,y,1), 1, nz*4*my, trigz, 1 )
           call rfft ( nx, nz,   ur(1,i,y,1), nz*4*my, 1, trigx, 1 )
      end do
...
!(*----------------------------  LEAVING PHYSICAL SPACE  --------------------------*)
      ...
      call rfft ( nx, nz, s2, nz, 1, trigx, -1 )
      do  z = 1, nz ;   s2(z,1) = cmplx(real(s2(z,1)),0);    end do
      call cfft ( nz, nx/2, s2, 1, nz, trigz, -1 )
  ...
  subroutine  phase3
    ...
    do  x = first_x, last_x
       ...
       do i = 1, 4
           call cfft ( ny, nz, u(1,i,x,1), nz*4*mx, 1, trigy, -1 )
       end do
...
subroutine   remesh
    ...
    do x = first_x, last_x
        ...
        do  i = 1,  3 
            ...
            call cfft ( nx+ny, nz, u2, 1, nx+ny, trigxy, 1 )
