...
case $arg_b in
    Release|CodeCoverage|Debug|RelWithDebInfo)
	info "Performing Release or CodeCoverage build"
	...
	if (which cmake >/dev/null 2>/dev/null && which mpif90 >/dev/null 2>/dev/null); then
	  ...
	  install_sh_PID=$!
          while ps -p $install_sh_PID > /dev/null; do
            info "Still working on installing opencoarrays and dependencies"
	...
	;;
    *)
	error "No such build type defined"
esac
