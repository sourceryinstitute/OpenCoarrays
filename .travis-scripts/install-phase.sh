...
if [ -n "${GIT_TAG:-}" ]; then # tagging a release
  info "Tagged relese detected! Installing \`makeself\` to pack self extracting archives."
  brew_install makeself # Script to create self extracting tar archives with shell headers
fi
...
_script_files=("install.sh")
for f in install_prerequisites/*; do
  _script_files+=("$f")
done
info "Script files:"
echo "${_script_files[@]}" | tr ' ' '\n'
_install_script_touched=false
for _file_name in "${_script_files[@]}"; do
    if grep "^$_file_name\$" <<< $FILES_CHANGED >/dev/null ; then
      info "Detected changes to install scripts"
      _install_script_touched=true
      break # No need to continue loop
    fi
done
if ! $_install_script_touched; then
...
if ([[ "X$arg_b" = "XScript" ]] && ($_install_script_touched || ${FORCE_PREREQ_BUILD:-false})); then
  ...
  if [[ "X$MY_OS" = "Xosx" ]]; then
    ...
    mpich="https://github.com/sourceryinstitute/opencoarrays/files/303249/mpich-3.2_1.yosemite.bottle.1.tar.gz"
    openmpi="https://github.com/sourceryinstitute/opencoarrays/files/303286/open-mpi-1.10.2_1.yosemite.bottle.1.tar.gz"
    info "Fetching custom MPI bottle from: ${!arg_m}"
...
brew list
[ "$LOG_LEVEL" -ge 7 ] && brew doctor || true
